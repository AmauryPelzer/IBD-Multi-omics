...
mbxCount.CD <- NoMissingData %>%
  select(HMDB, Metabolite, all_of(metaData.CD$External.ID))
mbxCount.UC <- NoMissingData %>%
  select(HMDB, Metabolite, all_of(metaData.UC$External.ID))
mbxCount.nonIBD <- NoMissingData %>%
  select(HMDB, Metabolite, all_of(metaData.nonIBD$External.ID))
...
# Check that no columns was lost
setdiff(metaData.nonIBD$External.ID, colnames(mbxCount.log.nonIBD))
setdiff(metaData.CD$External.ID, colnames(mbxCount.log.CD))
setdiff(metaData.UC$External.ID, colnames(mbxCount.log.UC))

#remove(metaData.CD, metaData.UC, metaData.nonIBD, mbxCount, NoMissingData, mbxCount.CD, mbxCount.UC, mbxCount.nonIBD)
```
