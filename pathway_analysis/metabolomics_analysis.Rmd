...
for(i in 1 : nrow(mbxCount.nonIBD)) { # For each gene : 
	x = mbxCount.nonIBD[i,c(3:ncol(mbxCount.nonIBD))] # control of gene number i
	y = mbxCount.CD[i,c(3:ncol(mbxCount.CD))] # CD of gene number i
...
for(i in 1 : nrow(mbxCount.nonIBD)) { # For each gene : 
	...
	y = mbxCount.UC[i,] # UC of gene number i
...
write.table(mbxCount$HMDB, "data/HMDB.csv", sep = ",", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
...
# Remove genes which has all zero values for all samples then start DE analysis
mbxCount[is.na(mbxCount)] <- 0
nonzero <- rowSums(mbxCount[,4:ncol(mbxCount)]) > 0
mbxCount <- mbxCount[nonzero,]
...
# Select metagenomics data from CD, UC and nonIBD
mbxCount.CD <- subset(mbxCount[,4:ncol(mbxCount)], select=metaData.CD$External.ID)
mbxCount.UC <- subset(mbxCount[,4:ncol(mbxCount)], select=metaData.UC$External.ID)
mbxCount.nonIBD <- subset(mbxCount[,4:ncol(mbxCount)], select=metaData.nonIBD$External.ID)
...
#remove(pValue.UC, t, x, y, i, mbxCount.nonIBD)
```
